...
const GATHERING_STATUS_BADGE_MAP: Partial<
  Record<keyof typeof GatheringsText.STATUS_TEXT_MAP, BadgeVariants['variant']>
> = {
  OPEN: 'default',
  UPCOMING: 'secondary',
  CLOSED: 'destructive',
...
const statusDisplay = computed(() =>
  TableDisplay.getMappedBadgeDisplay<
    keyof typeof GatheringsText.STATUS_TEXT_MAP,
    BadgeVariants['variant']
  >(
    props.status,
    GatheringsText.STATUS_TEXT_MAP,
...
/** 狀態 badge 的客製配色 */
const STATUS_BADGE_CLASS_MAP: Partial<Record<keyof typeof GatheringsText.STATUS_TEXT_MAP, string>> = {
...
<template>
  <!-- 顯示活動狀態的 Badge（文字與樣式皆由 status 計算而來） -->
